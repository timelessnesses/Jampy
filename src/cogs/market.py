...
class Selling(commands.Cog):
    ...
    async def sell(self, ctx, item, amount):
        ...
        if db[str(ctx.author.id)]["material"][item.lower()] >= amount:
            ...
            await ctx.send(embed=discord.Embed(title="Successfully sold {} {}(s)".format(amount, item), color=0xFFAA00))
        else:
            await ctx.send(embed=discord.Embed(title="You don't have enough {}(s)".format(item), color=0xFF0000))
